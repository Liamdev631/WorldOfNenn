...
private:
    ...
    public:
        ...
        Chunk(const tmx::TileLayer& layer, std::vector<const tmx::Tileset*> tilesets,
            const sf::Vector2u& position, const sf::Vector2u& tileCount, std::size_t rowSize,  TextureResource& tr)
        {
            ...
            for (const auto ts : tilesets)
            {
                ...
                for (auto y = position.y; y < position.y + tileCount.y; y++)
                {
                    for (auto x = position.x; x < position.x + tileCount.x; x++)
                    {
                            ...
                            && tileIDs[idx].ID < (ts->getFirstGID() + ts->getTileCount()))
                        {
                            ...
                            unsigned int idIndex = tileIDs[idx].ID - ts->getFirstGID();
                            ...
                            Tile tile = 
                            {
                                ...
                                sf::Vertex(tileOffset + sf::Vector2f((float)tileSize.x, 0.f), vertColour, tileIndex + sf::Vector2f((float)tileSize.x, 0)),
                                ...
                                sf::Vertex(tileOffset + sf::Vector2f(0.f, (float)tileSize.y), vertColour, tileIndex + sf::Vector2f(0, (float)tileSize.y))
#endif
#ifdef __ANDROID__								
								sf::Vertex(tileOffset, vertColour, tileIndex),
    ...
    void createChunks(const tmx::Map& map, const tmx::TileLayer& layer, tmx::IntRect bounds)
    {
       ...
       // m_chunkCount.x = static_cast<sf::Uint32>(std::ceil(bounds.width / m_chunkSize.x));
		...
		m_chunks.emplace_back(std::unique_ptr<Chunk>(new Chunk(layer, usedTileSets, 
            sf::Vector2u(bounds.left, bounds.top),
			sf::Vector2u(bounds.width, bounds.height),
    ...
    void updateVisibility(const sf::View& view) const
    {
        ...
        for (auto y = posY; y < posY + 2; ++y)
        {
            ...
            {
                unsigned int idx = y * int(m_chunkCount.x) + x;
